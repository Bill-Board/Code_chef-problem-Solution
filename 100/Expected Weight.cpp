...
v99 f(v99 a)
{
    return modAdd(fact[a],MOD);
}
v99 alabu(v99 a,v99 b)
{
    return modMul(a,inv[b]);
}
v99 sum(v99 a,v99 len)
{
    v99 p=modAdd(a,modMul(a,len));
    v99 mot=modMul(len,p);
    return modAdd(alabu(mot,2LL),MOD);
}
...
void solve()
{
    /// code is here->
    v99 n;read(n);
//    v99 in=modularInverse(n,MOD);
//    print(inv[n],in,modMul(n,in));
    v99 a=f(n-1);
    v99 ek=modAdd(sum(a,n),MOD);
    v99 ans=modAdd(sum(ek,n),MOD);
    print(modAdd(alabu(ans,n),MOD),inv[n],abs(modAdd(alabu(ans,n),MOD)-inv[n]));
}
