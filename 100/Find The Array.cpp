...
{
    v99 cr_h=0;
    rep0(i,0,a.size())cr_h+=abs(a[i]-b[i]);
    return cr_h;
}

vll alabu1(vll a, v99 n)
{
    vll ans1(n,1LL),ans2(n,1LL);
    for(v99 i=0;i<n;i+=2)ans1[i]=a[i];
    for(v99 i=1;i<n;i+=2)ans2[i]=a[i];
    v99 ok1=val(a,ans1),ok2=val(a,ans2);
    if(ok1<ok2)return ans1;
    else return ans2;
}
