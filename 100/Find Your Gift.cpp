...
{
    /// code is here->
    v99 n,m=0,x=0,y=0;read(n);
    char last[n+2];
    rep0(i,0,n)
    {
        char c;read(c);
        if(last[m]!=c)last[++m]=c;
    }
    rep1(i,1,m)
    {
        if(last[i]=='L' and last[i-1]!='R')--x;
        if(last[i]=='R' and last[i-1]!='L')x++;
        if(last[i]=='U' and last[i-1]!='D')++y;
        if(last[i]=='D' and last[i-1]!='U')--y;
    }
    print(x,y);
