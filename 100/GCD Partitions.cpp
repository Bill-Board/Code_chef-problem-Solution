...
///-------------------------------------------------------------------------------------------------------
v99 n,k,a[252],g[252],ans=0,dp[252][252],p=0;
void dfs(v99 koita,v99 ind,v99 sum)
{
    if(koita==k)
    {
        sum+=g[ind+1];
    ...
    v99 gd=0;
    v99 loop=n-k+koita;
    for(v99 i=ind+1;i<=loop;i++)
    {
        gd=gcd(gd,a[i]);
        dfs(koita+1,i,sum+gd);
    }
}
...
void solve()
{
    ...
    cin>>n>>k;
    rep1(i,1,n)cin>>a[i];
    for(v99 i=n;i>0;i--)g[i]=gcd(a[i],g[i+1]);
    dfs(1LL,0LL,0LL);
    cout<<ans<<ses;cout<<p<<ses;
}
