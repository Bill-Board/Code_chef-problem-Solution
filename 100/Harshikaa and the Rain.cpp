...
void solve()
{
    ...
    read(a);
    dfs(1LL,0LL);
    vll pre(lef.size()+1,0);
    sort(sob(lef));
    rep0(i,0,lef.size())
        pre[i+1]+=(pre[i]+lef[i]-1);
    //print(lef);print(pre);
    v99 cr_h=0,last=0;
    for(auto i:a)
    {
        v99 time=i-last;last=i;
        v99 pos=upper_bound(sob(lef),time)-lef.begin();
        cr_h+=max(0LL,(pos*time)-pre[pos]);
        //print(pre[pos],time);
        //print("pos ",pos,(pos*time)-pre[pos]);
    }
    print(cr_h);
}
