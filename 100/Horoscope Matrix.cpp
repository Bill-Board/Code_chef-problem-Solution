...
void solve()
{
    /// code is here->
    v99 n,m;read(n,m);
    v99 visi[n+m+4]={},koita=0;
    vector< map<v99,v99> >v(n+m+8);
    rep1(i,1,n)
    {
        rep1(j,1,m)
        {
    ...
    for(v99 i=2;i<=(n+m);i++)
    {
        if(v[i].size()>1)visi[i]=1,koita++;
    }
    v99 q;cin>>q;
    while(q--)
    {
        v99 x,y,val;read(x,y,val);y=berkor(y,m);
        v99 kot=x+y;
        auto j=v[kot].find(a[x][y]);
        if(j->ss==1)v[kot].erase(j);
        else v[kot][a[x][y]]--;

        a[x][y]=val;
        v[kot][val]++;
        if(v[kot].size()>1 && visi[kot]==0)
            koita++,visi[kot]=1;
        if(v[kot].size()==1 && visi[kot]==1) visi[kot]=0,koita--;
