...
void solve()
{
    ...
    rep0(i,1,n)c[i]=max(c[i],c[i-1]);
    d=a;
    rep0(i,1,n)a[i]=max(a[i],a[i-1]);
    for(v99 i=n-2;i>=0;i--)d[i]=max(d[i],d[i+1]);
    reverse(sob(c));
    v99 cr_h=-LLONG_MAX,p=-LLONG_MAX;
    for(v99 i=0;i<n-1;i++)
    {
        cr_h=max(1*b[i]+2*c[i+1],cr_h);
        p=max(p,1*a[i]+2*d[i+1]);
    }
    if(pos>1)print(cr_h);
    else if(pos==1)
    {
        if(ind==0)print(a[ind]+2*d[ind+1]);
        else if(ind==n-1)print(d[ind]+2*d[ind-1]);
        else print( max(a[ind]+2*d[ind+1],a[ind]*2+ d[ind+1]) );
    }
    else print(p);
}
