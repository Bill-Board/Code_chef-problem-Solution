...
vll alabu(vll a)
{
    ...
    for(auto i:a)
    {
        while(i>vp.back().ff)vp.pop_back();
        if(vp.back().ff!=i)vp.pb({i,0LL});
        ans.pb(vp.back().ss);
        vp.back().ss++;
    }
    return ans;
...
{
    /// code is here->
    v99 n;read(n);vll a(n),ans(n,0);read(a);
    vector<pii>vp;
    vll l=alabu(a);
    reverse(sob(a));
    vll r=alabu(a);
    reverse(sob(r));
    rep0(i,0,n)cout<<l[i]+r[i]<<whp ses[i==n-1];
    ...
    rep0(i,0,n)
    {
        if(!vp.empty() and vp.back().ff<a[i])
        {
            v99 val=mpp[vp.back().ff];
            mpp[vp.back().ff]=0;
            while(vp.back().ff<a[i] and !vp.empty())
            {
                ans[vp.back().ss]=val-1;
                vp.pop_back();
            }
        }
        mpp[a[i]]++;
    ...
    //while(!vp.empty())ans[vp.back().ss]=mpp[vp.back().ff]-1,vp.pop_back();
    print(ans);*/
}
