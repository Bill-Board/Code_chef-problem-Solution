...
///-------------------------------------------------------------------------------------------------------
v99 n,m;
v99 alabu(vector<string> &v,vector<string>&t)
{
    v99 cr_h=0,koita=0;
    for(v99 i=0;i<n;i++)
    {
        for(v99 j=0;j<m;j++)cr_h+=(v[i][j]!=t[i][j]),koita+=t[i][j]=='*';
    }
    if((n*m)&1 and koita*2<(n*m))cr_h=n*m;
    return cr_h;
}
...
void solve()
{
    /// code is here->
    read(n,m);
    vector<string>v(n),s(n),t(n);
    for(auto &i:v)read(i);
    rep0(i,0,m)
    {
        if(i&1)s[0].pb('*'),t[0].pb('.');
        else s[0].pb('.'),t[0].pb('*');
    }
    init(s);init(t);
    print(min(alabu(v,s),alabu(v,t)));
}
