...
v99 alabu()
{
    vll cnt(n+2);
    v99 cr_h=0;
    for(v99 i=m;i>0;i--)if(cnt[a[i]]==0)cnt[a[i]]=cr_h=m-i+1;
    v99 kot=0;cr_h=min(cr_h,vp.back().ss);
    while(vp.size()>1)
    {
        kot=max(cnt[vp.back().ff],kot);
        vp.pop_back();
        cr_h=min(vp.back().ss+kot,cr_h);
    }
    return cr_h;
}
...
void solve()
{
    ...
    {
        read(a[i]);
        if(s.find(a[i])==s.end())vp.pb({a[i],i});
        s.insert(a[i]);
    }//for(auto j:vp)print(j.ff,j.ss);print("");
    print(max(n,alabu()));
}
