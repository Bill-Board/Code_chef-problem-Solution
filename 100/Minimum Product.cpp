...
void alabu()
{
    ...
    for(v99 i=0;i<a.size();i++)
    {
        ...
        {
            v99 koita=0;
            while(m%j==0)koita++,m/=j;
            if(koita)s.insert(j),mpp[j].pb(koita);
            if(m==1)break;
            j==2?j+=1:j+=2;
        }
        if(m>1)s.insert(m),mpp[m].pb(1LL);
    }
    for(auto i:s)prm.pb(i);
}
...
void solve()
{
    ...
    for(auto i:prm)
    {
        ...
        while(pq.size()>1)
        {
            v99 last=pq.top();pq.pop();
            v99 fst=pq.top();pq.pop();
            if(last>1)pq.push(last-1);
            if(fst>1)pq.push(fst-1);
        }
        v99 sum=0;
        if(pq.size())sum=pq.top();
        cr_h=modMul(cr_h,modPow(i,sum));
    }
    print(cr_h);
}
