...
v99 mod=998244353LL;
vll fact(3*mxi,1LL),inv(3*mxi,1LL),ans(2*mxi);
void factorial(v99 n)
{
    for(v99 i=2;i<=n;i++)
    {
...
v99 alabu(v99 n,v99 x)
{
    ...
    v99 cr_h=1;
    for(auto i:mpp)cr_h=(cr_h*ncr(n+i.ss-1,i.ss))%mod;
    return cr_h;
}
...
{
    /// code is here->
    v99 n,m,cr_h=0;read(n,m);
    mpf(n+m+30);factorial(n+m+30);
    for(v99 i=1;i<=m;i++)cr_h=(cr_h+alabu(n,i))%mod;
    print(cr_h);
}
