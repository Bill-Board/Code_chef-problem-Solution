...
void solve()
{
    ...
    {
        v99 len=tom[i];
        if(len>1)vp.pb({a[i-1],len-1});
    }
    sort(sob(vp),sort2val);

    vll ans;ans.pb(0LL);
    for(auto i:vp)
    {
        v99 len=i.ss;
        while(len--)ans.pb(i.ff);
    }
    for(v99 i=1;i<ans.size();i++)ans[i]+=ans[i-1];
    for(auto i:ans)cout<<sum+i<<whp;
    cout<<ses;
}
