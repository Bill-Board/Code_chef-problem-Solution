...
void solve()
{
    ...
    b=a;
    sort(sob(a));
    v99 l=LLONG_MAX,r=-LLONG_MAX;
    multiset<v99> x,y;
    rep0(i,0,n)
    {
        l=min(l,a[i]);r=max(r,a[i]);
        if(x.find(b[i])!=x.end())x.erase(x.find(b[i]));
        else y.insert(b[i]);

        if(y.find(a[i])!=y.end())y.erase(y.find(a[i]));
        else x.insert(a[i]);

        if(x.empty() and y.empty())cr_h+=(r-l),r=0,l=LLONG_MAX;
    }
    print(cr_h);
}
