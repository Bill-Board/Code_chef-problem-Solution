...
void alabu(v99 n,v99 k)
{
    ...
    rep0(i,0,n+1)
    {
        v99 x;cin>>x;
        if(x&1)o.pb({x,i});
        else if(x!=0) e.pb({x,i});
    }
    for(v99 i=0;i<o.size();i+=2)
    {
        v99 val1=o[i].ff,clr1=o[i].ss;
        v99 val2=o[i+1].ff,clr2=o[i+1].ss;
        print(clr1,1LL,clr2,1LL);
        if(val1!=1)e.pb({val1-1,clr1});
        if(val2!=1)e.pb({val2-1,clr2});
    }
    for(auto i:e)
    {
        v99 val=i.ff,clr=i.ss;
        while(val>0)
        {
            print(clr,2LL,(clr+1)%(n+1),0LL);
            val-=2;
        }
    }
}
...
{
    /// code is here->
    v99 n,k;read(n,k);map<pii,v99>mpp;
    if(k==2){alabu(n,k);return;}
    vector<pii> a(n+1);
    rep0(i,0,n+1)cin>>a[i].ff,a[i].ss=i;
    sort(sob(a));
    v99 l=0,r=n;
    while(l<=r)
    {
        while(a[l].ff==0)l++;
        v99 val1=a[l].ff,clr1=a[l].ss;
        v99 val2=a[r].ff,clr2=a[r].ss;
        v99 dif=k-val1;
        if(dif>val2)
        {
            swap(a[l-1],a[r]);
            if(a[l].ff<a[l-1].ff)swap(a[l],a[l-1]);
            --l;
            --r;
            continue;
        }
        a[r].ff-=dif;
        print(clr1,val1,clr2,dif);
        l++;
        if(a[r].ff==0)r--;
    }
