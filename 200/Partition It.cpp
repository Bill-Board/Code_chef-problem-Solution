...
void mpf()
{
    ...
    for(v99 i=2;i<=N;i++)
    {
        ...
        for(v99 j=0;j<pr.size() and pr[j]<=lp[i] and i*pr[j]<=N;j++)
            lp[i*pr[j]]=pr[j];
    }
}
void solve()
{
    ...
    vll tom[n+1];
    v99 hlf=n/2;
    v99 pos=lower_bound(sob(pr),hlf)-pr.begin();
    tom[1].pb(1LL);
    set<v99>r;
    for(v99 i=max(0LL,pos-1);pr[i]<=n;i++)
        if(hlf<pr[i])tom[pr[i]].pb(pr[i]),r.insert(pr[i]);
    for(v99 i=2;i<=n;i++)
        if(r.find(i)==r.end())tom[2].pb(i);
    vector<pii>a;
    rep1(i,1,n)if(tom[i].size())a.pb({tom[i].size(),i});
    sort(sob(a));
    v99 sum=0;
    for(v99 i=0;i<a.size();i++)
    {
        ...
        if(n-sum==k)
        {
            print("YES");
            set<v99>s;
            for(v99 j=0;j<=i;j++)s.insert(a[j].ss);
            rep1(i,1,n)
            {
                if(s.find(i)==s.end())cout<<i<<whp;
            }
            cout<<ses;
            return;
        }
