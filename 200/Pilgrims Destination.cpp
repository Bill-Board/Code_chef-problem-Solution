...
vector<pii>tom[mxi];
vll ans;v99 visi[mxi];
///-------------------------------------------------------------------------------------------------------

void dfs(v99 ind,v99 val,v99 level)
{
    if(visi[ind])return;
    visi[ind]=1;
    bool ok=false;
    for(auto i:tom[ind])
    {
        if(visi[i.ff])continue;
        ok=true;
        dfs(i.ff,(level*i.ss)+val,level+1);
    }
    if(!ok)ans.pb(val);
}
...
{
    /// code is here->
    v99 n,m;read(n,m);vll a(m);
    read(a);
    sort(sob(a),greater<>());
    ans.clear();
    rep0(i,0,n+3)tom[i].clear(),visi[i]=0LL;
    rep0(i,0,n-1)
    {
        v99 u,v,k;read(u,v,k);
        tom[u].pb({v,k});
        tom[v].pb({u,k});
    }
    dfs(1LL,0LL,1LL);sort(sob(ans),greater<>());
    v99 cr_h=0;
    while(true)
    {
        if(ans.empty() or a.empty())break;
        v99 plg=a.back(),lagbe=ans.back();a.pop_back();
        if(plg>=lagbe)cr_h++,ans.pop_back();
    }
    print(cr_h);
}
