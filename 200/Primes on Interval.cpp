...
v99 SieveOfEratosthenes(v99 a,v99 n)
{
	...
	v99 cr_h=(a<=2 and n!=1);
	prm.pb(2LL);
	for(v99 i=3;i<=n;i+=2)
	{
	    if(!prime[i])prm.push_back(i);
	    if(a<=i  and prime[i])cr_h++;
	}
	return cr_h;
}
...
bool alabu(v99 a, v99 b, v99 k, v99 range)
{
    v99 cr_h=0;
    for(v99 i=a;i<range+a;i++)cr_h+=prime[i];
    v99 sum=cr_h;
    for(v99 l=a,r=a+range;r<=b;r++,l++)
    {
        cr_h+=prime[r];
        cr_h-=prime[l];
        sum=min(sum,cr_h);
...
{
    /// code is here->
    v99 a,b,k;read(a,b,k);
    if(SieveOfEratosthenes(a,b)<k){print(-1);return;};
    /// using tow pointer 1st choise
    ///rep0(i,a,b)cout<<i<<whp<<prime[i]<<ses;
    v99 l=1,r=b-a+1;
    while(l<r)
    {
        v99 mid=(l+r)>>1;
        if(alabu(a,b,k,mid))r=mid;
        else l=mid+1;
    }
    print(r);
}
