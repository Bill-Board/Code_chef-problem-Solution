...
void solve()
{
    ...
    v99 sum=0,cr_h=0,mod=998244353LL,last_sum=0,last_val=0;
    sort(sob(a));reverse(sob(a));
    for(v99 i=0;i<n;i++)
    {
        v99 kot=(((a[i]*a[i])%mod)+(a[i]*(last_sum*2 + last_val))%mod)%mod;
        cr_h=(cr_h+kot)%mod;
        last_sum=(last_sum*2LL +last_val)%mod;
        last_val=a[i];
    }
    print(cr_h);
}
