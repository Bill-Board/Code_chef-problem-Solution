...
void solve()
{
    ...
    map<v99,v99>mpp;
    vector<pii>b,cnt(n);
    rep0(i,0,n)cnt[i].ff=0,cnt[i].ss=i;
    for(auto &i:a)read(i),mpp[i]++;
    rep1(i,1,n)
    {
        v99 mid1=(i+1)/2,mid2=(n-i+1+n)/2;
        cnt[mid1-1].ff++;cnt[mid2-1].ff++;
    }
    for(auto i:mpp)b.pb({i.ss,i.ff});
    sort(sob(b));sort(sob(cnt));
    if(b.size()!=cnt.size())return print(-1);
    for(v99 i=0;i<n;i++)
    {
        if(b[i].ff!=cnt[i].ff)return print(-1);
        ans[cnt[i].ss]=b[i].ss;
    }
    if(!is_sorted(sob(ans)))return print(-1);
    rep0(i,1,n)if(ans[i]<=ans[i-1])return print(-1);
    print(ans);
}
