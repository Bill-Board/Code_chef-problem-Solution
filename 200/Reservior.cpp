...
void solve()
{
    ...
    rep0(i,0,n)
    {
        bool blc=false,air=false;
        rep0(j,0,m)
        {
            if(s[i][j]=='B')
            {
                if(s[i+1][j]!='B')return print("no");
            }
            else if(s[i][j]=='W')
            {
                if(s[i+1][j]!='B' and s[i+1][j]!='W')return print("no");
                if(j==0 or j==m-1)return print("no");
                if (s[i][j - 1] == 'A' || s[i][j + 1] == 'A')return print("no");
            }
        }
    }
    print("yes");
