...
///-------------------------------------------------------------------------------------------------------
vll a,tom[mxi];
v99 cr_h=0,n,gone[mxi],gtwo[mxi];
void all_clear(v99 n)
{
    a.resize(n+3);cr_h=0;
    rep1(i,0,n+5)
    {
...
void dfs2(v99 ind ,v99 par)
{
    ...
    {
        if(i==par)continue;
        p.pb(gcd(p.back(),gone[i]));
        e.pb(i);
        g+=gone[i];
    }


    for(v99 i=tom[ind].size()-1;i>=0;i--)
    {
        if(tom[ind][i]==par)continue;
        s.pb(gcd(s.back(),gone[tom[ind][i]]));
    }
    reverse(sob(s));
    for(v99 i=1;i<p.size();i++)
        gtwo[e[i-1]]=gcd(gtwo[ind],gcd(a[ind],gcd(p[i-1],s[i])));

//    print("ind  ",ind);
//    print(tom[ind]);
//    print(p);print(s);
//    rep1(i,1,n)cout<<gtwo[i]<<whp;cout<<ses;
//    print(g,gtwo[ind],ses,ses);
    cr_h=max(cr_h,g+gtwo[ind]);
...
void solve()
{
    ...
    rep1(i,1,n)read(a[i]);
    dfs1(1LL,0LL);dfs2(1LL,0LL);
//    rep1(i,1,n)cout<<gone[i]<<whp;cout<<ses;
//    rep1(i,1,n)cout<<gtwo[i]<<whp;cout<<ses;
    print(cr_h);
}
