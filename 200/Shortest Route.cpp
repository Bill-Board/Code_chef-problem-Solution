...
void solve()
{
    /// code is here->
    v99 n,m;read(n,m);
    vll a(n+1),pre(n+2),suf(n+2);
    rep1(i,1,n)
    {
        read(a[i]);
        if(a[i]==1)pre[i]=i;
        else if(a[i]==2)suf[i]=i;
    }
    for(v99 i=2;i<=n;i++)if(!pre[i] and pre[i-1])pre[i]=pre[i-1];

    for(v99 i=n-1;i>0;i--)if(!suf[i] and suf[i+1])suf[i]=suf[i+1];
    while(m--)
    {
        v99 x;read(x);
        if(x==1)cout<<0<<whp;
        else if(!pre[x] and !suf[x])cout<<-1<<whp;
        else if(!pre[x])cout<<abs(suf[x] - x)<<whp;
        else if(!suf[x])cout<<abs(pre[x] - x)<<whp;
        else cout<<min(abs(pre[x]-x),abs(suf[x]-x))<<whp;
    }
    cout<<ses;
}
