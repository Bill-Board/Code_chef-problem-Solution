...
{
    /// code is here->
    string s;read(s);v99 n=s.size();
    set<string>ans;
    rep0(i,0,n)
    {
        string q="",t="",u="";v99 one=0,st=i;
        while(s[st]=='1' and st<n)
            u+=s[st],t+=s[st++],ans.insert(t);
        //cout<<"yes "<<u<<ses;
        rep0(j,st,n)
        {
            one+=(s[j]=='1');//cout<<one<<ses;
            if(one==2)
            {
                if(q.back()=='1')
                {
                    u.pop_back();q.pop_back();
                    t+="11";
                    u=t+q;
                    one=0;
                }
