...
void solve()
{
    ...
    for(auto c:w)
    {
        for(auto i:mpp)
        {
            if(i.ff==c.ff)continue;
            for(auto j:i.ss)
            {
                string s=j;
                s[0]=c.ff;
                if(p.find(s)==p.end())ans.insert(s);
            }
        }
    }
    //for(string i:ans)cout<<i<<whp;cout<<ses;
    map<char,set<string> >suf;
    ...
    //print(w);print(k);
    //for(auto i:suf){print(i.ff);for(auto j:i.ss)cout<<j<<whp;cout<<ses;}
    //for(auto i:pre){print(i.ff);for(auto j:i.ss)cout<<j<<whp;cout<<ses;}
    for(auto i:suf)//if(w[i.ff]==1)
        cr_h+= ( i.ss.size()* (i.ss.size()-1));
    for(auto i:pre)//if(k[i.ff]==1)
    cr_h+= ( i.ss.size()* (i.ss.size()-1));
    print(max(0LL,(v99)(ans.size()*(ans.size()-1))-cr_h));
}
