...
///-------------------------------------------------------------------------------------------------------
v99 a[1003][1003],r[1003][1003],c[1003][1003];
void init()
{
    for(v99 i=1;i<=1000;i++)
    {
        a[i][1]=a[i-1][1]+i;
        r[i][1]=a[i][1];
        c[i][1]+=(c[i-1][1]+a[i][1]);
        for(v99 j=2,lp=i;j<=1000;j++,lp++)
        {
            a[i][j]=a[i][j-1]+lp;
            r[i][j]+=(r[i][j-1]+a[i][j]);
            c[i][j]+=(a[i][j]+c[i-1][j]);
...
{
    /// code is here->
    v99 x1,x2,y1,y2;read(x1,y1,x2,y2);
    v99 sum=c[x2][y1] -c[x1-1][y1];
    v99 sum2=r[x2][y2]-r[x2][y1-1];
    print(sum2+sum-a[x2][y1]);
}
