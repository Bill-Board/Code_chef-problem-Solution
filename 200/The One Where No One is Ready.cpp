...
///-------------------------------------------------------------------------------------------------------
void asd(deque<v99>d)
{
    while(d.size())cout<<d.front()<<whp,d.pop_front();
    cout<<ses;
}
...
v99 alabu(char c)
{
    ...
    for(v99 i=0;i<n;i++)
    {
        ...
        if(kot)a.push_back(kot);
    }
    if(s.back()!=c)a.push_back(0LL);
    print(a);
    v99 sum=0;
    for(v99 i=1;i<=k and a.size();i++)
    {
        v99 val=a.back();a.pop_back();
        sum+=val;
    }
    if(a.size()==1)sum+=a.back();
    //print("SUM ",sum);
    return sum;
}
...
void solve()
{
    ...
    v99 cr_h=0;
    set<char>p;
    for(auto i:s)p.insert(i);
    if(k==0 and p.size()>1)return print(-1);
    if(k==0)return print(n);
    for(auto i:p)cr_h=max(cr_h,alabu(i));
    print(cr_h);
}
