...
///-------------------------------------------------------------------------------------------------------
string s;
v99 dist(v99 a,v99 b)
{
    return abs(a-b);
}

pii alabu(v99 x,v99 y,char c)
{
    for(auto i:s)
    {
        if(i=='0')
...
{
    /// code is here->
    v99 n,p,q;read(n,p,q,s);
    pii a=alabu(0LL,0LL,'x'),b=alabu(0LL,0LL,'y');
    //print(a.ff,a.ss);
    //print(b.ff,b.ss);
    v99 ek=dist(0LL,p),dui=dist(0LL,a.ff);
    v99 ek1=dist(0LL,q),dui1=dist(0LL,a.ss);
    if(ek<=dui and abs(ek-dui)%2==0 and ek1<=dui1 and abs(ek1-dui1)%2==0)
        return print("YES");
    ek=dist(0LL,p);dui=dist(0LL,b.ff);
    ek1=dist(0LL,q);dui1=dist(0LL,b.ss);
    //print(ek,dui,ek1,dui1);
    if(ek<=dui and abs(ek-dui)%2==0 and ek1<=dui1 and abs(ek1-dui1)%2==0)
        return print("YES");
    if((a.ff==p and a.ss==q) or (b.ff==p and b.ss==q ))print("YES");
    else print("NO");
}
