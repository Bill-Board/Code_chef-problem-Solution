...
///-------------------------------------------------------------------------------------------------------
v99 n,ans,koita=0,sum=0,visi[mxi],l[mxi];
string s;
vll e,tom[mxi];
void bfs()
{
...
void solve()
{
    ...
    s="_"+s;
    v99 one=0;
    rep1(i,1,n)one+=(s[i]=='1');
    e.clear();
    for(v99 i=0;i<=n+10;i++)
    {
        tom[i].clear();
        visi[i]=l[i]=0LL;
    }
    ...
    for(auto i:e)
    {
        for(v99 i=0;i<=n+2;i++)visi[i]=0LL;
        ok=false;
        ans=i;
        dfs(i);
        if(one<=koita)break;
