...
///-------------------------------------------------------------------------------------------------------
v99 od,ev;
bool alabu(v99 x,v99 y)
{
    if(y&1)return true;y/=2;
    for(v99 i=1;i*i<=y;i++)
    {
        if(y%i==0)
        {
            v99 even=i,odd=(y/i);
            v99 sum=((even*(even-1))+even)+ (odd*(odd-1))+odd;
            if(sum==x)
            {
                od=odd;ev=even;
                return false;
...
void solve()
{
    /// code is here->
    v99 x,y;read(x,y);
    if(alabu(x,y) or ev+od<1)return print("NO");
    print("YES");
    if(od<ev)swap(od,ev);
    print(od+ev);
    print(1,2);od--;ev--;
    v99 i=3;
    while(ev>0)print(1,i++),ev--;
    while(od>0)print(2,i++),od--;
}
