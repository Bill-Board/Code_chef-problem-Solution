...
v99 n,k,a[4002],dp[4001][4001],ans=LLONG_MAX;

void dfs(v99 ind,v99 tracy,v99 mike)
{
    //print(ind-1,tracy,mike);
    if(tracy<1 and mike<1){ans=min(ind-1,ans);return;}
    if(ind>n)return;
    if(tracy>0)dfs(ind+1,tracy-a[ind],mike);
    if(mike>0)dfs(ind+1,tracy,mike-a[ind]);
}
...
{
    /// code is here->
    read(n,k);ans=LLONG_MAX;
    rep1(i,1,n)cin>>a[i];
    sort(a+1,a+n+1,greater<>());
    //rep1(i,1,n)cout<<a[i]<<whp;cout<<ses;
    dfs(1LL,k,k);
    if(1<ans and ans<=n)print(ans);
    else print(-1);
}
