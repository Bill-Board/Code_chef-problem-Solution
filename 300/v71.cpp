...
void solve()
{
    /// code is here->
    string a,b;read(a,b);
    if(a.size()!=b.size()){print(0);return;}
    v99 l=0,n=a.size();
    while(a[l]==b[l])l++;
    if(l<(n-1)){print(0);return;}
    v99 lasta=a[a.size()-1]-'0',lastb=b[b.size()-1]-'0';
    if(lasta>lastb){print(0);return;}
    v99 cr_h=1;
    for(v99 i=lasta+1;i<=lastb;i++)
    {
        cr_h*=i;
        cr_h%=10;
    }
