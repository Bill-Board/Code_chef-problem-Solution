...
{
    /// code is here->
    v99 n,a,k;read(n,k,a);
    vll f(k),ans,ok;
    read(f);
    sort(sob(f));
    for(v99 i=1;i<n;i++){v99 u,v;read(u,v);}
    for(v99 b=1;b<=n;b++)
    {
        if(a<=b)
        {
            if(b<=f.back())ans.pb(b-a);
            else if(a>=f.back())ans.pb(a-b);
            else ans.pb( (f.back()-a)-(b-f.back()) );
            ok.pb(f.back());
        }
        else
        {
            if(b>=f.front())ans.pb(a-b);
            else if(a<=f.front())ans.pb(b-a);
            else
            {
                auto val=upper_bound(sob(f),b);
                ans.pb((a-*val)-(*val-b));
            }
            ok.pb(f.front());
        }
