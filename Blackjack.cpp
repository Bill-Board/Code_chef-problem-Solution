...
bool ek(map<v99,v99>mpp,vll a,v99 sum,v99 pos,v99 del)
{
    v99 ok=sum;
...
bool dui(map<v99,v99>mpp,vll a,v99 pos,v99 del,v99 ok)/// kot>y
{
     for(v99 k=pos;k;k--)
        {
            ok-=a[k];
            if(x<=ok and y<=ok){print(1);return;}
...
void solve()
{
    ...
    bool ok=true;
    for(v99 i=x;i<=y;i++)if(bit[i])ok=false;
    ...
    {
        sum+=a[i];
        if(sum>=x and sum<=y){print(0);return;}
        if(sum>y)
        {
            for(v99 j=i+1;j<n;j++)mpp[a[j]]++;
            for(v99 j=0;j<=pos;j++)
            {
                v99 ok=sum-a[j];
                if(x<=ok and y<=ok){print(1);return;}
                ...
                else if(ek(mpp,a,ok,i,j)){print(1);return;}
            }
            break;
        }
    }
