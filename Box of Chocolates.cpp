...
void init(v99 node,v99 b, v99 e)
{
    ...
    init(left,b,mid);
    init(right,mid+1,e);
    tre[node]=max(tre[right],tre[left]);
}

v99 query(v99 node, v99 b, v99 e, v99 i, v99 j)
{
    if(i>e || j<b)return 0;
    ...
    v99 right=node*2 +1;
    v99 mid=(e+b)/2;
    v99 p1=query(left,b,mid,i,j);
    v99 p2= query(right,mid+1,e,i,j);
    return max(p1,p2);
}
void solve()
{
    ...
    rep1(i,n+1,2*n)arr[i]=arr[i-n];
    v99 l=1LL,r=n/2 ,cr_h=0;
    init(1LL,1LL,2*n);///cout<<query(1LL,1LL,2*n,x,y)<<ses;
    v99 m=query(1LL,1LL,2*n,1LL,2*n);
    while(l<=n)
    {
        //print(l,r);
        if(m!=query(1LL,1LL,2*n,l,r))cr_h++;
        ++l;++r;
    }
    print(cr_h);
