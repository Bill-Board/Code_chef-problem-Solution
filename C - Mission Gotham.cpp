...
{
    /// code is here->
    v99 n;read(n);vll a(n+2);root.resize(n+3);
    rep1(i,1,n)read(a[i]),root[i]=i;root[n+1]=n+1;
    v99 q;read(q);
    while(q--)
    {
        v99 x,k,cr_h=0;read(x,k);
        v99 kot=findroot(x),i;
        for(i=kot;i<=n;i=findroot(i+1))
        {
            v99 length=i-x;//print(i);
            if(a[i]>=k)
            {
                a[i]-=k;
                cr_h+=(length*k);
		...
		print(cr_h);
		//print(a);print(root);print(ses,ses);
    }
}
