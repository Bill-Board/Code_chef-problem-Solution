...
void solve()
{
    /// code is here->
    string s;read(s);
    v99 n=s.size(),cr_h=0,last=0;
    if(n<2){print(0);return;}
    for(v99 i=0;i<n-2;)
    {
        if(s[i]=='0'){i++;continue;}
        if(s[i]==s[i+1] and s[i]==s[i+2])cr_h+=2,s[i+2]='0',i+=3;
        else if(s[i]==s[i+1])cr_h++,s[i+1]='0',i+=2;
        else if(s[i]==s[i+2])cr_h++,s[i+2]='0',i++;
        else i++;
    }
    //print(last);
    //print(s);
    if(s[n-1]==s[n-2] and s[n-1]!='0')cr_h++;
    print(cr_h);
}
