...
void solve()
{
    ...
    rep1(i,1,k)tom[i].pb(n+3);
    v99 cnt=0;
    rep1(i,2,n)cnt+=(b[i]!=b[i-1]);
    vll p;
    for(v99 i=1;i<=k;i++)
    {
        vll a;
        ...
        {
            if(a.empty())a.pb(j);
            else if(a.back()==j-1)a.pb(j);
            else
            {
                v99 low=a.front(),up=a.back();
                if(low==1 and up!=n)
                {
                    ans--;
                }
                else if(low!=1 and up==n)
                {
                    ans--;
                }
                else if(low!=1 and up!=n)
                {
                    if(b[low-1]!=b[up+1])ans--;
                    else ans-=2;
