...
/// 1st way
v99 n,e,h,a,b,c;
v99 alabu(v99 dim,v99 choc,v99 ache)
{
    if(!ache)return 0LL;
    v99 nibo=dim/2;
    v99 baki=max(0LL,ache-nibo);
    if(baki*3>choc or dim<2)return LLONG_MAX;
    else return (min(nibo,ache)*a) + (baki*b);
}

v99 alabu2(v99 dim,v99 choc,v99 ache)
{
    if(!ache)return 0LL;
    v99 nibo=choc/3;
    v99 baki=max(0LL,ache-nibo);
    if(baki*2>dim or choc<3)return LLONG_MAX;
    else return (min(nibo,ache)*b) + (baki*a);
}
...
void solve()
{
    ...
    read(n,e,h,a,b,c);
    v99 cr_h=LLONG_MAX;
    for(v99 i=0;i<=min({h,e,n});i++)
    {
        v99 ache=n-i;
        v99 dim=e-i,choc=h-i;
        v99 d=min(alabu(dim,choc,ache),alabu2(dim,choc,ache));
        if(d!=LLONG_MAX)cr_h=min(d+(i*c),cr_h);
    }
    if(cr_h==LLONG_MAX)print(-1);
    else print(cr_h);
}
