...
void solve()
{
    /// code is here->
    v99 n,m;read(n,m);
    vll a(n),b(m);read(a);read(b);
    while(a.size()!=b.size())
    {
        if(a.size()>b.size())b.pb(mxi);
        else a.pb(mxi);
    }b.pb(mxi);a.pb(mxi);
    reverse(sob(b));reverse(sob(a));
    //print(a);print(b);
    bool foot=true;
    v99 cr_h=0;
    while(a.back()!=mxi or b.back()!=mxi)
    {
        if(foot)
        {
            if(b.back()<a.back())cr_h++,foot=false,b.pop_back();
            else a.pop_back();
        }
        else if(!foot)
        {
            if(b.back()>a.back())cr_h++,foot=true,a.pop_back();
            else b.pop_back();
        }
        //print(a);print(b);
    }
