...
void inst()
{
    for(v99 i=0,q=1;i<26;i++,q*=2)
    {
        mpp[q]=s[i];
    }
}
v99 alabu(v99 n,v99 len)
{
    queue<v99>q;q.push(n);
    vll a;
    while(q.size()+a.size()!=len)
    {
        v99 val=q.front();q.pop();
        if(val==1){a.pb(1LL);continue;}
        q.push(val/2);q.push(val/2);
    }
    while(!q.empty())cout<<mpp[q.front()],q.pop();
    for(auto i:a)cout<<mpp[i];
}
...
void solve()
{
    ...
    if(a.size()>n or k<n){print(-1);return;}
    v99 kot=n-a.size()+1;
    alabu(a[0],kot);
    for(v99 i=1;i<a.size();i++)cout<<mpp[a[i]];
    cout<<ses;
}
