...
{
    /// code is here->
    v99 n,cr_h=LLONG_MAX;read(n);
    vll a(n);
    multiset<v99>s,ms;
    map<v99,v99>mpp;
    for(auto &i:a)read(i),s.insert(i),ms.insert(i);
    sort(sob(a));
    rep0(i,0,n)
    {
        if(i==0)mpp[a[i]]=-LLONG_MAX;
        else if(a[i]==a[i-1])mpp[a[i]]=mpp[a[i-1]];
        else mpp[a[i]]=a[i-1];
    }

    //for(auto i:mpp)print(i.ff,i.ss);
    v99 ek=a.front(),dui=a.back();
    for(v99 i=n-2;i>=0;i--)
    {
        v99 dif1=abs(ek- a[i]),lagbe=dui-dif1,last=a[i+1];
        v99 val=ms.upper_bound(a.begin(),a.begin()+i,lagbe),kp=mpp[val];
        if(ms.find(kp)==ms.end())kp=mpp[kp];
        if(last>=val)cr_h=min(cr_h,abs(lagbe-val));
        if(last>=kp)cr_h=min(cr_h,abs(lagbe-kp));
    }
    for(auto i=s.rbegin();i!=s.rend();i++)
    {

        //print(*i,dif1,lagbe,val,kp,last);
        ms.insert(*i);
        last=*i;
    }
    print(cr_h);
